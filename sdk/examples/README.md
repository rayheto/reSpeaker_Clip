...
| `demo_stream_fft_display.py` | Capture + real-time terminal FFT spectrum of an RTC stream through the SDK `JitterBuffer` + Opus decoder + NumPy FFT, with an adaptive noise floor and hard spectral gate.|
...
Install the BLE and play extra (see `sdk/README.md` for full install instructions; skip if already installed):
